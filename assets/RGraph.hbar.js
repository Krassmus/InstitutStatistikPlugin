...
{
this.id = id;this.canvas = document.getElementById(id);this.context = this.canvas.getContext ? this.canvas.getContext("2d") : null;this.canvas.__object__ = this;this.data = data;this.type = 'hbar';this.coords = [];this.isRGraph = true;RGraph.OldBrowserCompat(this.context);this.max = 0;this.stackedOrGrouped = false;this.properties = {
...
{
RGraph.FireCustomEvent(this, 'onbeforedraw');RGraph.ClearEventListeners(this.id);this.gutterLeft = this.Get('chart.gutter.left');this.gutterRight = this.Get('chart.gutter.right');this.gutterTop = this.Get('chart.gutter.top');this.gutterBottom = this.Get('chart.gutter.bottom');this.coords = [];this.max = 0;if(this.Get('chart.xmin') > 0 && this.Get('chart.grouping') == 'stacked'){
...
{
e = RGraph.FixEventObject(e);var canvas = document.getElementById(this.id);var obj = canvas.__object__;var bar = obj.getBar(e);var mouseCoords = RGraph.getMouseXY(e);if(bar && (typeof(obj.Get('chart.tooltips')) == 'function' || obj.Get('chart.tooltips')[bar[4]]) ){
canvas.style.cursor = 'pointer';var left = bar[0];var top = bar[1];var width = bar[2];var height = bar[3];var idx = bar[4];if(obj.Get('chart.tooltips.event') == 'onmousemove'){
var tooltipObj = RGraph.Registry.Get('chart.tooltip');if(typeof(obj.Get('chart.tooltips')) == 'function'){
...
RGraph.HideTooltip();RGraph.Redraw();obj.context.beginPath();obj.context.strokeStyle = obj.Get('chart.highlight.stroke');obj.context.fillStyle = obj.Get('chart.highlight.fill');obj.context.strokeRect(left, top, width, height);obj.context.fillRect(left, top, width, height);RGraph.Tooltip(canvas, text, e.pageX, e.pageY, idx);}
return;}
}
}
if(!bar){
...
this.canvas.addEventListener('mousemove', canvas_onmousemove_func, false);RGraph.AddEventListener(this.id, 'mousemove', canvas_onmousemove_func);var canvas_onclick_func = function (e)
{
e = RGraph.FixEventObject(e);var canvas = e.target;var obj = canvas.__object__;var bar = obj.getBar(e);RGraph.Redraw();var mouseCoords = RGraph.getMouseXY(e);if(bar){
var left = bar[0];var top = bar[1];var width = bar[2];var height = bar[3];var idx = bar[4];if(typeof(obj.Get('chart.tooltips')) == 'function'){
var text = obj.Get('chart.tooltips')(idx);} else if(typeof(obj.Get('chart.tooltips')) == 'object' && typeof(obj.Get('chart.tooltips')[idx]) == 'function'){
var text = obj.Get('chart.tooltips')[idx](idx);} else if(typeof(obj.Get('chart.tooltips')) == 'object'){
var text = obj.Get('chart.tooltips')[idx];} else {
var text = null;}
if(String(text).length && text != null){
obj.context.beginPath();obj.context.strokeStyle = obj.Get('chart.highlight.stroke');obj.context.fillStyle = obj.Get('chart.highlight.fill');obj.context.strokeRect(left, top, width, height);obj.context.fillRect(left, top, width, height);obj.context.stroke();obj.context.fill();RGraph.Tooltip(canvas, text, e.pageX, e.pageY, idx);}
}
e.stopPropagation();}
this.canvas.addEventListener('click', canvas_onclick_func, false);RGraph.AddEventListener(this.id,'click', canvas_onclick_func);if(RGraph.Registry.Get('chart.tooltip')){
...
this.context.moveTo(halfway, this.gutterTop);this.context.lineTo(halfway, RGraph.GetHeight(this) - this.gutterBottom);} else {
...
this.context.moveTo(this.gutterLeft, RGraph.GetHeight(this) - this.gutterBottom);this.context.lineTo(RGraph.GetWidth(this) - this.gutterRight, RGraph.GetHeight(this) - this.gutterBottom);var yTickGap = (RGraph.GetHeight(this) - this.gutterTop - this.gutterBottom) / this.data.length;for (y=this.gutterTop; y<(RGraph.GetHeight(this) - this.gutterBottom); y+=yTickGap){
if(this.Get('chart.yaxispos') == 'center'){
this.context.moveTo(halfway + 3, y);this.context.lineTo(halfway  - 3, y);} else {
this.context.moveTo(this.gutterLeft, y);this.context.lineTo( this.gutterLeft  - 3, y);}
}
xTickGap = (RGraph.GetWidth(this) - this.gutterLeft - this.gutterRight ) / 10;yStart = RGraph.GetHeight(this) - this.gutterBottom;yEnd = (RGraph.GetHeight(this) - this.gutterBottom) + 3;for (x=(RGraph.GetWidth(this) - this.gutterRight), i=0; this.Get('chart.yaxispos') == 'center' ? x>=this.gutterLeft : x>this.gutterLeft; x-=xTickGap){
...
this.context.beginPath();this.context.fillStyle = this.Get('chart.text.color');if(this.Get('chart.yaxispos') == 'center'){
...
{
var obj = e.target.__object__;var canvas = obj.canvas;var context = obj.context;var mouseCoords = RGraph.getMouseXY(e);for (var i=0,len=obj.coords.length; i<len; i++){
