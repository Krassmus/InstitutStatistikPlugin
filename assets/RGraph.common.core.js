...
{
var obj = canvas.__object__;var context = canvas.getContext('2d');var gutterLeft = obj.Get('chart.gutter.left');var gutterRight = obj.Get('chart.gutter.right');var gutterBottom = obj.Get('chart.gutter.bottom');var size = arguments[4] ? arguments[4] : 12;var centerx = (arguments[3] ? arguments[3] : ((RGraph.GetWidth(obj) - gutterLeft - gutterRight) / 2) + gutterLeft);var keypos = obj.Get('chart.key.position');var vpos = obj.Get('chart.title.vpos');var hpos = obj.Get('chart.title.hpos');var bgcolor = obj.Get('chart.title.background');if(obj.type == 'bar' && obj.Get('chart.variant') == '3d'){
keypos = 'gutter';}
...
var size = obj.Get('chart.text.size');var font = obj.Get('chart.text.font');context.beginPath();RGraph.Text(context, font, size + 2, RGraph.GetWidth(obj) / 2, RGraph.GetHeight(obj) - (gutterBottom * obj.Get('chart.title.xaxis.pos')), obj.Get('chart.title.xaxis'), 'center', 'center', false, false, false, true);context.fill();}
if(typeof(obj.Get('chart.title.yaxis')) == 'string' && obj.Get('chart.title.yaxis').length){
var size = obj.Get('chart.text.size');var font = obj.Get('chart.text.font');var angle = 270;var yaxis_title_pos = obj.Get('chart.title.yaxis.pos');if(obj.Get('chart.title.yaxis.position') == 'right'){
angle = 90;yaxis_title_pos = yaxis_title_pos * obj.Get('chart.gutter.right') + (obj.canvas.width - obj.Get('chart.gutter.right'));} else {
...
context.moveTo(gutterLeft, y);context.lineTo(RGraph.GetWidth(obj) - gutterRight, y);}
}
...
context.moveTo(x, gutterTop);context.lineTo(x, RGraph.GetHeight(obj) - gutterBottom);}
}
if(obj.Get('chart.background.grid.border')){
context.strokeStyle = obj.Get('chart.background.grid.color');context.strokeRect(gutterLeft, gutterTop, RGraph.GetWidth(obj) - gutterLeft - gutterRight, RGraph.GetHeight(obj) - gutterTop - gutterBottom);}
}
...
var linewidth = obj.Get('chart.crosshairs.linewidth');context.lineWidth = linewidth ? linewidth : 1;context.beginPath();context.strokeStyle = obj.Get('chart.crosshairs.color');context.moveTo(x, gutterTop);context.lineTo(x, height - gutterBottom);context.moveTo(gutterLeft, y);context.lineTo(width - gutterRight, y);context.stroke();if(obj.Get('chart.crosshairs.coords')){
if(obj.type == 'scatter'){
