...
var canvas = obj.canvas;var context = obj.context;var resizeHandle = 15;RGraph.Resizing.canvas = canvas;RGraph.Resizing.placeHolders = [];if(!canvas.__original_width__ && !canvas.__original_height__){
...
canvas.style.cursor = 'move';if(navigator.userAgent.indexOf('Chrome') > 0 || document.all){
canvas.title = 'Resize the graph';}
} else if(   coords[0] > (canvas.width - resizeHandle - resizeHandle)
&& coords[0] < canvas.width - resizeHandle
&& coords[1] > (canvas.height - resizeHandle)
&& coords[1] < canvas.height){
canvas.style.cursor = 'pointer';if(navigator.userAgent.indexOf('Chrome') > 0 || document.all){
canvas.title = 'Reset graph to original size';}
} else {
canvas.style.cursor = 'default';canvas.title = '';}
}
canvas.addEventListener('mousemove', canvas_onmousemove, false);RGraph.AddEventListener(canvas.id, 'mousemove', canvas_onmousemove);var canvas_onmouseout = function (e)
{
e.target.style.cursor = 'default';e.target.title = '';}
canvas.addEventListener('mouseout', canvas_onmouseout, false);RGraph.AddEventListener(canvas.id, 'mouseout', canvas_onmouseout);var canvas_onmousedown = function (e)
{
e = RGraph.FixEventObject(e);var coords = RGraph.getMouseXY(e);var canvasCoords = RGraph.getCanvasXY(e.target);var canvas = e.target;if(   coords[0] > (obj.canvas.width - resizeHandle)
&& coords[0] < obj.canvas.width
&& coords[1] > (obj.canvas.height - resizeHandle)
&& coords[1] < obj.canvas.height){
RGraph.FireCustomEvent(obj, 'onresizebegin');if(canvas.__original_css_border__ == null){
canvas.__original_css_border__ = canvas.style.border;}
RGraph.Resizing.mousedown = true;var div = document.createElement('DIV');div.style.position = 'absolute';div.style.left = canvasCoords[0] + 'px';div.style.top = canvasCoords[1] + 'px';div.style.width = canvas.width + 'px';div.style.height = canvas.height + 'px';div.style.border = '1px dotted black';div.style.backgroundColor = 'gray';div.style.opacity = 0.5;div.__canvas__ = e.target;document.body.appendChild(div);RGraph.Resizing.div = div;RGraph.Resizing.placeHolders.push(div);for (var i=0; i<(RGraph.Resizing.placeHolders.length - 1); ++i){
...
&& coords[1] > (canvas.height - resizeHandle)
&& coords[1] < canvas.height){
RGraph.FireCustomEvent(canvas.__object__, 'onresizebegin');canvas.width = canvas.__original_width__;canvas.height = canvas.__original_height__;canvas.style.border = canvas.__original_css_border__;canvas.style.left = (parseInt(canvas.style.left)) + 'px';canvas.style.top = (parseInt(canvas.style.top)) + 'px';RGraph.FireCustomEvent(canvas.__object__, 'onresizebeforedraw');canvas.__object__.Draw();if(RGraph.Resizing.div){
